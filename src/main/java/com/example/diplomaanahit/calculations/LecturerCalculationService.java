...
public class LecturerCalculationService {
    ...
    public Double calculateLecturers(Set<Lecturer> lecturers) {
        ...
        for (Lecturer lecturer : lecturers) {
            if(lecturer.getLevel().equals("professor")) {
                factor += 1;
            }
            else if(lecturer.getLevel().equals("associate professor")) {
                factor += 0.75;
            }
            else if(lecturer.getLevel().equals("assistant professor")) {
                factor += 0.5;
            }
            else if(lecturer.getLevel().equals("lecturer")) {
                factor += 0.25;
            }
        }
